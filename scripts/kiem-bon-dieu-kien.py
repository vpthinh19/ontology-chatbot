"""Kiểm dataset theo bốn điều kiện người dùng nêu 2026-08-15.

Chạy:  .venv/bin/python scripts/kiem-bon-dieu-kien.py
...
Nó CHẠY THẬT từng truy vấn trên đồ thị chứ không đọc bằng mắt. Hai chỗ từng tự
đánh lừa mình khi viết cái này: regex bắt ngày tháng nhận cả số bốn chữ số trần
nên "Quy chế 1052" cũng khớp; và phép so đích với danh mục dùng nhầm
``target_template`` nên báo 3.907/3.907 đích "ngoài danh mục".
"""
...
catalogue = {}
for line in (DATASET / "catalogue.jsonl").read_text(encoding="utf-8").splitlines():
    if line.strip():
...
# ĐÃ BỎ phép "đích SELECT có nằm trong danh mục không".
#
# Nó so chuỗi đích với ``target_template`` của danh mục - mà template còn nguyên
# chỗ trống ``${anchor}`` chưa thay - nên báo TOÀN BỘ đích là "ngoài danh mục".
# Suýt thành một phát hiện giả nghiêm trọng.
#
# Bỏ luôn chứ không sửa, vì đã có phép canh thật ở chỗ khác:
# ``validate_sparql_dataset`` đối chiếu độ phủ giữa danh mục và ba tập, và bộ sinh
# chỉ dựng đích TỪ danh mục nên không có đường nào đẻ ra đích ngoài đó.
