...
OUT = ROOT / "reports" / "bao-cao.html"

# Ba slot đầu của bảng màu chuẩn. Chỉ ba, vì đó là tập con đã được chứng minh
# đạt mọi ngưỡng phân biệt màu ở CẢ hai chế độ sáng/tối kể cả khi so từng cặp.
# Chỗ nào cần nhiều nhóm hơn thì dùng MỘT màu: ở đó màu không mang danh tính,
# nhãn trên trục mới mang.
...
def ontology_shape() -> dict:
    ...
    return {
        # Hai con số khác nhau, và phải nói rõ là khác: ``triples`` là số bộ ba
        # NGƯỜI VIẾT trong tệp; lúc chạy còn dựng thêm một "chiếu nguồn" để truy
        # vấn khỏi lặp lại đoạn dò nguồn dài. Báo con số runtime như thể đó là
        # công sức biên soạn là nói quá.
        "triples": len(g),
...
def benchmarks() -> list[dict]:
    found = []
    for path in sorted((ROOT / "metrics").glob("benchmark-*.json")):
...
def bars(items, *, color=0, pct=False, width=560) -> str:
    ...
    Nhãn trực tiếp chứ không phải chú giải màu: các nhóm ở đây phân biệt bằng
    CHỮ trên trục, không bằng màu, nên một màu là đúng - thêm màu chỉ tạo ra
    thông tin giả.
    """
