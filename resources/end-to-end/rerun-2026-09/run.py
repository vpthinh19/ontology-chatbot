"""Đo lại bộ đề giữ kín trên hệ thống hiện tại: hỏi chatbot 70 câu, lưu câu trả lời và dữ liệu công cụ.

Bản sao của ``../heldout/run.py`` (bộ đề đó niêm phong cùng báo cáo, không sửa, không ghi vào). Khác bản
gốc: kết quả ghi vào thư mục này; khi dịch vụ mô hình vẫn quá tải sau các lần gọi lại của LightningClient
(``LightningBusyError``) thì chờ rồi hỏi lại cả câu; lưu dòng đánh dấu mô hình viết (``danh_dau``) để đo độ
chính xác của nó. ``questions.json`` chép nguyên từ bộ niêm phong: ngày 19/09/2026 đã dò lại nhãn
``ontology`` của 17 câu "văn bản có, ontology không/một phần" với ontology hiện tại, không câu nào đổi.
Chỉ chạy một lượt (``--luot 1``) vì chi phí mô hình.
...
so với đáp án gốc trong ``questions.json``.
    ...
    set -a; . ./.env; set +a
    uv run python resources/end-to-end/rerun-2026-09/run.py --luot 1            # ghi results-1.json
    CHAY_BU=1 uv run python resources/end-to-end/rerun-2026-09/run.py --luot 1  # chỉ hỏi lại câu bị lỗi
"""
...
def cho_khi_bi_gioi_han(exc: LightningBusyError, lan: int) -> float:
    """Số giây chờ khi dịch vụ vẫn quá tải: theo lời dịch vụ, và không dưới 30 giây nhân số lần đã thử."""
