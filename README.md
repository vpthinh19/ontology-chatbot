...
Agent là cửa vào duy nhất của mỗi yêu cầu. Hai cache nằm bên trong công cụ
`tra_cuu_hoc_vu`; trình duyệt và tầng HTTP/SSE không gọi thẳng bộ phân loại hay
đồ thị.

- Cache phân loại ánh xạ từ khoá đã chuẩn hoá tới kết quả chọn truy vấn cụ thể đã
  được kiểm định. Kết quả không có thông tin cũng được cache, nhưng lỗi thì không.
- Cache SPARQL thuộc về đúng phiên bản ontology bất biến đã nạp trong tiến trình.
  Khoá của nó gồm câu `SELECT` cụ thể đã kiểm định và `max_rows`; giá trị là các
  dòng kết quả đóng băng. Kết quả rỗng thành công được cache, còn ngoại lệ, lỗi
  kiểm định, vượt giới hạn dòng và kết quả dở dang thì không.
- Cache miss luôn chạy chính câu SPARQL cụ thể đã kiểm định trên ontology. Cache
  không thay thế SPARQL làm nguồn dữ kiện, và câu trả lời cuối của LLM không bao
  giờ được cache.
- Hai cache chỉ nằm trong bộ nhớ của một tiến trình. Khởi động lại dịch vụ hoặc
  nạp bản model/ontology/catalogue mới dựng lại graph, coordinator và cache rỗng;
  `ONTCHATBOT_MODEL_REVISION` chỉ là nhãn chẩn đoán, không tham gia quyết định
  cache đúng hay sai.
- Đặt ngân sách của cache tương ứng thành `0` để tắt lưu giá trị hoàn tất và quay
  về đường thực thi không cache. Singleflight vẫn hoạt động để các yêu cầu lạnh
  trùng nhau cùng chờ một lần tính, thay vì đồng thời tạo nhiều native work.
...
Các giá trị dưới đây là mặc định của lệnh `serve_sparql`; cờ dòng lệnh cùng tên
(ví dụ `--lookup-workers`) ghi đè biến môi trường. Worker/thread và turn slots là
hồ sơ ban đầu: phải đo lại trên đúng CPU quota của môi trường triển khai, không
suy ra từ tên hay dòng sản phẩm CPU.

| Biến môi trường | Mặc định | Ý nghĩa |
|---|---:|---|
| `ONTCHATBOT_ONNX_THREADS` | `1` | Số intra-op thread của một ONNX inference |
| `ONTCHATBOT_LOOKUP_WORKERS` | `8` | Số native lookup chạy đồng thời trong executor |
| `ONTCHATBOT_TURN_SLOTS` | `16` | Số lượt Agent được nhận chạy đồng thời |
| `ONTCHATBOT_TURN_QUEUE` | `64` | Số lượt tối đa chờ admission |
| `ONTCHATBOT_CLASSIFICATION_CACHE_ENTRIES` | `4096` | Số kết quả phân loại hoàn tất tối đa; `0` để tắt |
| `ONTCHATBOT_SPARQL_CACHE_MIB` | `64` | Ngân sách payload SPARQL hoàn tất, tính bằng MiB; `0` để tắt |
| `ONTCHATBOT_MODEL_REVISION` | `unknown` | Nhãn revision chỉ dùng trong log chẩn đoán |

Kiểm tra tính đúng trước khi đo hiệu năng:

```bash
uv run pytest -n 0 tests/runtime -q
uv run pytest -q
uv run validate_sparql_dataset
```

Chạy từng ứng viên lạnh trong tiến trình mới trên deployment host:

```bash
uv run benchmark_runtime --model-dir "$ONTCHATBOT_MODEL_DIR" \
  --workload resources/end-to-end/results.json --onnx-threads 2 \
  --lookup-workers 4 --concurrency 4 --rounds 5
uv run benchmark_runtime --model-dir "$ONTCHATBOT_MODEL_DIR" \
  --workload resources/end-to-end/results.json --onnx-threads 1 \
  --lookup-workers 8 --concurrency 8 --rounds 5
```

Đo cache ấm, singleflight khi nhiều yêu cầu trùng nhau, rồi độ ổn định bộ nhớ:

```bash
uv run benchmark_runtime --model-dir "$ONTCHATBOT_MODEL_DIR" \
  --workload resources/end-to-end/results.json --onnx-threads 1 \
  --lookup-workers 8 --concurrency 32 --rounds 20 --warm
uv run benchmark_runtime --model-dir "$ONTCHATBOT_MODEL_DIR" \
  --workload resources/end-to-end/results.json --onnx-threads 1 \
  --lookup-workers 8 --concurrency 32 --rounds 1 --warm --duration 3600
```

Mỗi lần so sánh phải giữ nguyên host, CPU quota, model, ontology và workload;
kiểm tra `errors`, throughput thành công, p95 độ trễ/event-loop lag, native
submissions, hit/follower, trọng lượng cache và RSS. Không dùng số đo ở máy phát
triển làm giá trị triển khai khi chưa có baseline trước thay đổi trên cùng host.
