...
def test_parse_query_nested():
    ...
    assert c.act == QUERY
    assert c.goc.label == "học phí" and c.goc.loai == INDIVIDUAL
    assert c.goc.con[0].label == "k65"


def test_acts_without_tree():
    assert parse({"act": "greeting", "entities": []}).goc is None
...
def test_ghost_children_dropped():
    ...
    assert [n.label for n in c.goc.con] == ["ok"]
...
def test_data_node_strips_children():
    ...
    leaf = c.goc.con[0]
    assert leaf.loai == DATA and leaf.con == ()
