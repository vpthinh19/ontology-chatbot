...
from ontchatbot.benchmark import (
    BenchmarkError,
    evaluate_benchmark,
    load_benchmark,
...
from ontchatbot.dataset import load_dataset
from ontchatbot.query_engine import load_ontology


def test_frozen_benchmark_is_balanced_and_executable() -> None:
    rows = load_benchmark()
    report = validate_benchmark(rows, load_ontology(), training_rows=load_dataset())
...
def test_frozen_manifest_checksums_match() -> None:
    manifest_path = Path("resources/benchmarks/sparql_v1_manifest.json")
...
def test_benchmark_rejects_exact_training_leak() -> None:
    rows = load_benchmark()
    leaked = dict(rows[0])
    leaked["input"] = load_dataset()[0]["input"]

    with pytest.raises(BenchmarkError, match="leaks from training"):
        validate_benchmark([leaked], load_ontology(), training_rows=load_dataset())
