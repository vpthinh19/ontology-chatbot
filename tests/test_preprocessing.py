"""Tests for :class:`ontchatbot.preprocessor.Preprocessor`."""

from __future__ import annotations

import pytest

from ontchatbot.preprocessor import Preprocessor
...
def test_get_returns_singleton():
    assert Preprocessor.get() is Preprocessor.get()
...
def test_normalize_does_not_expand_teencode(pre: Preprocessor):
    """Dataset path must NOT expand teen-code (training data is already expanded)."""
    out = pre.normalize("hp k65")
    assert "hp k65" in out
    assert "học phí" not in out


def test_normalize_for_match_strips_diacritics_and_punct(pre: Preprocessor):
    assert pre.normalize_for_match("Đăng ký môn học!") == "dang ky mon hoc"
    assert pre.normalize_for_match("Phòng CTSV") == "phong ctsv"
...
def test_is_url_static():
    assert Preprocessor.is_url("https://example.com")
    assert Preprocessor.is_url("http://example.com")
    assert not Preprocessor.is_url("example.com")
    assert not Preprocessor.is_url(123)
