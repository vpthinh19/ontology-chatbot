...
Sau khi mô hình sinh ra cây JSON, chặng duyệt *thi hành* cây đó: bắt đầu từ gốc, đi dần theo từng nút con, biến đổi một **tập hiện
tại** cho tới khi ra đáp án.

Mục này đi qua bốn ý, từ đơn giản đến đầy đủ: (1) khớp một nhãn cho điểm thế nào, (2) tập hiện tại biến đổi ra sao, (3) một ví dụ
duyệt trọn vẹn, (4) các trường hợp biên.

**Mỗi nút có một nhãn và một vai trò.** Vai trò quyết định nhãn được đem so với phần nào của ontology:
...
là một — dưới đây viết gọn `bao luu`.
...
**Một cá thể có nhiều bề mặt; điểm của nó là bề mặt cao điểm nhất.** Ví dụ nhãn gốc `bao luu` chấm trên các cá thể cùng chứa cụm này:
...
**Chọn cá thể ở đỉnh.** Đỉnh là 100 và chỉ `QuyTrinhBaoLuu` chạm tới, nên gốc là `QuyTrinhBaoLuu`. (Khi đỉnh bị nhiều cá thể chia
nhau, xem *Trường hợp biên* ở cuối mục.)

**Nút quan hệ và nút thuộc tính** cũng cho điểm như vậy, nhưng so với *nhãn đã khai báo* của quan hệ/thuộc tính. Ví dụ nhãn "phòng
xử lý" (`phong xu ly`) trùng khít nhãn của quan hệ `duocXuLyBoi` → 100; nhãn "email" trùng khít nhãn thuộc tính `email` → 100.
...
Cách bố trí nút trong cây quyết định phép hợp thành — đây chính là chỗ ontology làm được điều mà tìm kiếm văn bản thường không làm được:
...
Cả hai cấu trúc sẽ thấy rõ ở ví dụ học phí bên dưới.
...
**Trường hợp biên.** Mạch trên giả định mọi nhãn khớp gọn vào đúng một đích. Bốn tình huống lệch được xử lý riêng, mỗi tình huống
cho một phản hồi xác định:
...
- **Hai hệ chấm theo lối khác nhau**: ontology chấm micro, hệ phẳng chấm trung bình theo câu — nên khi so "đúng trọn vẹn cả tập"
  phải đặt đúng cặp exact-set ↔ full@k.
