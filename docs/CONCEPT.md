...
Trên tập kiểm tra gồm 2.044 câu, mô hình đạt exact match accuracy trung bình theo năm nhóm năng lực là 0,96 và macro-F1 cũng
0,96. Mọi câu đều sinh JSON hợp lệ và đúng hợp đồng cây. Riêng các câu truy vấn đạt F1 0,96, exact match 0,96 và độ chính xác
phân loại `act` 1,00, gần như không sai ý định. Tách theo năm nhóm năng lực, kết quả đều ở mức cao và phản ánh đúng độ khó: đi
nhiều bước 0,99, lọc theo ràng buộc 0,98, nhiều thuộc tính 0,97, tra cứu trực tiếp 0,95 và đi một quan hệ 0,94 (F1). Các loại
phi-truy-vấn được báo cáo riêng: câu ngoài tri thức đạt F1 0,90, câu có gốc rơi vào lớp hay quan hệ trần (đáng lẽ trả lời "không
hiểu") đạt 0,91, và câu mơ hồ đạt 0,93.
...
|---|---|---|---|---|
| Ontology (đầu-cuối, mô hình thật) | 0,96 | — | — | 0,98 (trùng khít tập) |
| Phẳng | 0,46 (@1) | 0,70 | 0,80 | 0,68 (full@3) |
...
|---|---|---|---|---|---|
| Tra cứu trực tiếp | 444 | 0,98 | 0,98 | 0,98 | 1,00 |
| Đi một quan hệ | 642 | 0,95 | 0,97 | 0,11 | 0,44 |
| Đi nhiều bước | 308 | 1,00 | 1,00 | 0,19 | 0,72 |
| Nhiều thuộc tính | 134 | 0,98 | 0,96 | 0,50 | 0,56 |
| Lọc theo ràng buộc | 225 | 0,98 | 0,97 | 0,80 | 0,93 |
| **Toàn bộ** | **1.753** | **0,97** | **0,98** | **0,46** | **0,70** |
...
thuộc tính (0,56) và đi một quan hệ (0,44), trong khi ontology giữ recall quanh 0,90 đến 1,00 ở mọi nhóm — đây là bằng chứng trực
tiếp cho luận điểm trung tâm.

![Hình 14](figures/recall_at_k.png)

**Hình 14.** Đường recall@k của hệ phẳng (0,46 ở k=1 lên 0,80 ở k=5). Recall tăng khi nới k nhưng vẫn nằm dưới mốc recall 0,96 của
...
0,87 (nhóm nhiều thuộc tính) đến 0,99 (đi nhiều bước) cho cả việc tìm đúng thuộc tính và đúng giá trị, phần lớn trên 0,95. Hệ phẳng thuần truy hồi không trích thuộc tính nên không áp dụng được ở tầng này; đây là khác biệt bản chất chứ không
