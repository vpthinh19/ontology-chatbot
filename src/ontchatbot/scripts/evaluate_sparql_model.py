...
from .train_sparql import MAX_SOURCE_LENGTH, MAX_TARGET_LENGTH
...
def evaluate(args: argparse.Namespace) -> dict:
    ...
    graph = load_ontology()
    training_rows = load_dataset()
    validate_dataset(training_rows, graph)
    ...
    if args.suite in {"validation", "both"}:
        rows = [row for row in training_rows if row["split"] == "validation"]
